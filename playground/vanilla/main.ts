...
on("info", () => {
	...
	note(
		"playground.deferred",
		"info: waiting on the record builder and the public logger",
	);
});
on("action", () => {
	...
	note(
		"playground.deferred",
		"logAction: waiting on the record builder and the public logger",
	);
});
on("metric", () => {
	...
	note(
		"playground.deferred",
		"logMetric: waiting on the metrics stream and the public logger",
	);
});
on("error", () => {
	...
	note(
		"playground.deferred",
		"logger.error: waiting on the record builder and the public logger",
	);
});
...
on("journey-start", () => {
	...
	note(
		"playground.deferred",
		"startJourney: waiting on the journey engine and the public API",
	);
});
on("journey-end", () => {
	...
	note(
		"playground.deferred",
		"endJourney: waiting on the journey engine and the public API",
	);
});
on("open-window", () => {
...
on("fetch-500", () => {
	...
	void fetch("http://localhost:8787/does-not-exist")
		.then((res) => {
			note(
				"playground.fetch_done",
				`mock server answered ${String(res.status)}`,
			);
		})
		.catch((err: unknown) => {
...
on("burst", () => {
	...
	note(
		"playground.deferred",
		"burst of 250: waiting on batching and the durable queue",
	);
});
...
on("offline", () => {
	...
	void fetch(control, { method: "POST" })
		.then(() => {
			note(
				"playground.server_forced",
				`server forced failure: ${String(failing)}`,
			);
		})
		.catch((err: unknown) => {
